...
{
  return 22 + 7*m_linCombSize + (m_decodedTableSize + m_decodingTableSize) / BITS_PER_CHAR;
}
                      ...
                      if (m_myNodeId == it -> second.GetDestination()) // We have received a packet at destination !!!!!
                        {
                          ...
                          NS_LOG_UNCOND ("t = "<< now.GetSeconds ()<<" "<<" the key "<<it -> first<<" have received in "<<m_myNodeId<<" destination ! and delivery delay for this packet is : "<<(now.GetNanoSeconds () - g->m_genTime));
                        }
...
MyNCApp::GeneratePacket ()
{
	...
	{
		uint8_t destId = uniVar->GetInteger(0,nNode-1);
		Time now = Simulator::Now ();
		if (m_buffer.size ()!=BUFFER_SIZE) {
			...
			nc->m_genTime = now.GetNanoSeconds ();
